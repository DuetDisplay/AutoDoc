...
const RETRYABLE_ERROR_PATTERNS = ['0x8009002d', 'SignerSign() failed', 'unexpected internal error']
...
function resolveSmctlPath() {
  ...
  try {
    const resolved = execFileSync('where.exe', ['smctl'], {
      encoding: 'utf8',
    ...
    // Fall through to the standard installation locations below.
  }
...
exports.default = async function sign(configuration) {
  ...
  if (!smctlPath || !existsSync(smctlPath)) {
    if (requireSigning) {
      throw new Error(
        'Windows signing is required, but smctl was not found on PATH or in the standard DigiCert install locations.'
      )
    }
    return
  }
