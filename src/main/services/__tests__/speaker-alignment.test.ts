...
function makeTranscript(
  overrides: Partial<Transcript> & { startMs: number; endMs: number }
): Transcript {
  return {
    id: 'test',
    meetingId: 'meeting-1',
...
describe('alignSpeakers', () => {
  ...
  it('labels segments as "me" when system audio is silent (with diarization)', () => {
    ...
    const diarization: DiarizationResult = {
      speakers: [{ id: 'SPEAKER_00', segments: [{ start: 3.0, end: 6.0 }] }]
    }
  ...
  it('returns transcripts unchanged when diarization is null and no mic segments', () => {
    const transcripts: Transcript[] = [makeTranscript({ id: 't1', startMs: 0, endMs: 5000 })]
