/**
 * PROBE2 usability probe (hybrid only) — runs the SECOND 50 unseen scenarios in
 * `ask-ai-probe2-scenarios.ts` through the same `chat:send-stream` entry point in
 * HYBRID mode (the shipping default) against the same 4 fixtures. Second
 * independent estimate of how often a real user hits an edge case vs a correct
 * answer on input we did NOT design the system around.
 *
 * Gated + skipped unless AUTODOC_LIVE_EVAL=1 (real Ollama round-trips):
 *   AUTODOC_LIVE_EVAL=1 AUTODOC_ASK_AI_MODEL=llama3.1 \
 *   npm run test:main:run -- src/main/ipc/__tests__/ask-ai-probe2.live.test.ts
 */
...
import { PROBE2_SCENARIOS } from '../../services/__tests__/ask-ai-probe2-scenarios'
...
const REPORT_DIR = join(process.cwd(), 'artifacts', 'ask-ai-probe2-eval')

vi.mock('electron', () => ({
  app: { getPath: vi.fn(() => join(tmpdir(), 'autodoc-ask-ai-probe2-userdata')) },
...
describe.skipIf(!LIVE)('Ask AI PROBE2 usability probe (hybrid)', () => {
  let baseDir: string
  ...
  beforeAll(async () => {
    ...
    baseDir = await mkdtemp(join(tmpdir(), 'autodoc-ask-ai-probe2-'))
  ...
  afterAll(async () => {
    ...
    const lines: string[] = [
      '',
      `════════ Ask AI PROBE2 usability probe (mode=hybrid, model=${MODEL}) ════════`,
  ...
  it('runs the probe2 scenario set in hybrid mode', async () => {
    ...
    for (const scenario of PROBE2_SCENARIOS) {
      senderId += 1
      ...
      for (let i = 0; i < scenario.turns.length; i++) {
        ...
        const requestId = `probe2-${scenario.id}-${i}`
        // A turn whose predicate is the trivial () => true is a conversational
        // setup step; exclude it from the scored denominator.
