...
export function FeatureStep({ icon, iconBg, heading, body, features, onNext }: FeatureStepProps) {
  return (
    <div className="text-center">
      <div
        className={`w-16 h-16 rounded-2xl ${iconBg} flex items-center justify-center text-[28px] mx-auto mb-5`}
      >
        {icon}
      </div>
      ...
      {features && (
        <div className="flex flex-col gap-3 mb-7 text-left">
          {features.map((f) => (
            <div
              key={f.title}
              className="flex items-start gap-3 px-4 py-3 bg-bg-card border border-border rounded-xl"
            >
              <div
                className={`w-9 h-9 rounded-lg ${f.iconBg} flex items-center justify-center text-[16px] shrink-0`}
              >
                {f.icon}
              </div>
              <div>
