...
Analytics are fully opt-in. During onboarding, the user is asked whether to enable anonymous usage data. The consent state is stored as:
...
- **Usage events**: Lightweight events like `search_performed` (with result count) and `calendar_connected`.
