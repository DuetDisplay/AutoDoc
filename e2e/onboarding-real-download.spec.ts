...
const RUN_MAC_MLX_REAL_SETUP_TEST = process.env.AUTODOC_RUN_MAC_MLX_REAL_SETUP_TEST === '1'
const REAL_SETUP_USER_DATA_DIR = process.env.AUTODOC_REAL_SETUP_USER_DATA_DIR
...
test.describe('real managed setup downloads', () => {
  test('macOS Apple Silicon clean install prepares MLX transcription successfully', async () => {
    test.skip(
      !RUN_MAC_MLX_REAL_SETUP_TEST,
      'Set AUTODOC_RUN_MAC_MLX_REAL_SETUP_TEST=1 to run real MLX setup validation.'
    )
    test.skip(
      process.platform !== 'darwin' || process.arch !== 'arm64',
      'This MLX setup integration test only runs on Apple Silicon macOS hosts.'
    )
    test.setTimeout(REAL_DOWNLOAD_TIMEOUT_MS)

    const app = await launchRealSetupApp(undefined, {
      userDataDir: REAL_SETUP_USER_DATA_DIR,
      cleanupUserDataDir: !REAL_SETUP_USER_DATA_DIR
    })
    ...
    try {
      ...
      const whisperStatus = await waitForSetupReady(
        page,
